package io.github.kuvandikov.scan_card_nfc;
...
import io.github.kuvandikov.scan_card_nfc.parser.EmvParser;
...
public class CardNfcAsyncTask extends AsyncTask<Void, Void, Object>{
    ...
    private CardNfcAsyncTask(Builder b) {
        ...
        if (mTag != null) {
            ...
            try {
                if ((mTag.toString().contains(NFC_ISO_DEP) && mTag.toString().contains(NFC_B_TAG)) ||
                        (mTag.toString().contains(NFC_ISO_DEP) && mTag.toString().contains(NFC_A_TAG))) {
                    execute();
                } else {
                    if (!b.mFromStart) {
