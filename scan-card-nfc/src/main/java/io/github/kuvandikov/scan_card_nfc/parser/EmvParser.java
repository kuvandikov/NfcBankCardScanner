...
import io.github.kuvandikov.scan_card_nfc.enums.CommandEnum;
import io.github.kuvandikov.scan_card_nfc.enums.EmvCardScheme;
...
import io.github.kuvandikov.scan_card_nfc.iso7816emv.EmvTerminal;
import io.github.kuvandikov.scan_card_nfc.iso7816emv.TLV;
import io.github.kuvandikov.scan_card_nfc.iso7816emv.TagAndLength;
import io.github.kuvandikov.scan_card_nfc.model.Afl;
import io.github.kuvandikov.scan_card_nfc.model.EmvCard;
import io.github.kuvandikov.scan_card_nfc.model.EmvTransactionRecord;
import io.github.kuvandikov.scan_card_nfc.model.enums.CurrencyEnum;
import io.github.kuvandikov.scan_card_nfc.utils.BytesUtils;
import io.github.kuvandikov.scan_card_nfc.utils.CommandApdu;
import io.github.kuvandikov.scan_card_nfc.utils.ResponseUtils;
import io.github.kuvandikov.scan_card_nfc.utils.TlvUtil;
import io.github.kuvandikov.scan_card_nfc.utils.TrackUtils;
...
public class EmvParser {
	...
	protected byte[] selectPaymentEnvironment() throws CommunicationException {
		...
		CommandApdu commandApdu = new CommandApdu(CommandEnum.SELECT, types, 0x00);

		return provider.transceive(commandApdu.toBytes());
	}
	...
	protected boolean readWithPSE() throws CommunicationException {
		...
		if (ResponseUtils.isSucceed(data)) {
			...
			if (ResponseUtils.isSucceed(data)) {
				...
				for (byte[] aid : aids) {
					String pApplicationLabel = extractApplicationLabel(data);
					ret = extractPublicData(aid, pApplicationLabel);
					if (ret == true) {
						break;
					}
				}
	...
	protected byte[] selectAID(final byte[] pAid) throws CommunicationException {
		...
		CommandApdu commandApdu = new CommandApdu(CommandEnum.SELECT, pAid, 0);
